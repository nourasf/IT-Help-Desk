...
public class ReportsController : ControllerBase
{
    ...
    public async Task<IActionResult> GetReport([FromQuery] DateTime? from, [FromQuery] DateTime? to)
    {
        ...
        // Use an exclusive end so the complete 'to' day is included.
        ...
        var openTickets = await ticketsInPeriod.CountAsync(t =>
            t.Status.StatusName == "Open" ||
            t.Status.StatusName == "In Progress" ||
            t.Status.StatusName == "Pending" ||
            t.Status.StatusName == "Reopened");

        var resolvedTickets = await ticketsInPeriod.CountAsync(t =>
            t.Status.StatusName == "Resolved");

        var closedTickets = await ticketsInPeriod.CountAsync(t =>
        ...
        var resolutionRate = totalTickets == 0
            ? 0
            : Math.Round((double)completedTickets / totalTickets * 100, 1);
        ...
        var resolutionMinutes = await ticketsInPeriod
            ...
            .Select(t => EF.Functions.DateDiffMinute(
                t.CreatedAt,
                t.ResolvedAt ?? t.ClosedAt!.Value))
            .Where(minutes => minutes >= 0)
            .ToListAsync();

        var averageResolutionMinutes = resolutionMinutes.Count == 0
            ? 0
            : Math.Round(resolutionMinutes.Average(), 1);

        var completedWorkSessions = _context.TicketWorkSessions
            .AsNoTracking()
            .Where(session =>
                session.StartAt >= fromDate &&
                session.StartAt < endExclusive &&
                session.EndedAt != null);

        var totalWorkMinutes = await completedWorkSessions
            .SumAsync(session => session.DurationMinutes ?? 0);

        var completedWorkSessionCount = await completedWorkSessions.CountAsync();
        var averageWorkMinutes = completedWorkSessionCount == 0
            ? 0
            : Math.Round((double)totalWorkMinutes / completedWorkSessionCount, 1);

        var ticketsByStatus = await ticketsInPeriod
        ...
        var ticketsByCategory = await ticketsInPeriod
        ...
        var ticketsByPriority = await ticketsInPeriod
        ...
        var ticketDates = await ticketsInPeriod
            .Select(t => t.CreatedAt.Date)
            .ToListAsync();

        var volumeLookup = ticketDates
            .GroupBy(date => date)
        ...
        var ticketsByDay = Enumerable.Range(0, dayCount)
            ...
            .Select(date => new
            {
                ...
                count = volumeLookup.TryGetValue(date, out var count) ? count : 0
            })
            .ToList();
        ...
        var agents = await _context.Users
            ...
            .Where(user =>
                user.Role != null &&
                (user.Role.Name == "IT Support Agent" ||
        ...
        {
            var assignedDuringPeriod = await _context.TicketAssignments
                .AsNoTracking()
                .CountAsync(assignment =>
                    assignment.AgentUserID == agent.ID &&
                    assignment.AssignedAt >= fromDate &&
                    assignment.AssignedAt < endExclusive);

            var resolvedDuringPeriod = await _context.Tickets
                .AsNoTracking()
                .CountAsync(ticket =>
                    !ticket.IsDeleted &&
                    ticket.AssignedToUserId == agent.ID &&
                    ((ticket.ResolvedAt.HasValue &&
                      ticket.ResolvedAt.Value >= fromDate &&
                      ticket.ResolvedAt.Value < endExclusive) ||
                     (ticket.ClosedAt.HasValue &&
                      ticket.ClosedAt.Value >= fromDate &&
                      ticket.ClosedAt.Value < endExclusive)));
            ...
            var activeTickets = await _context.Tickets
                ...
                .CountAsync(ticket =>
                    ...
                    ticket.Status.StatusName != "Cancelled");

            var agentWorkQuery = _context.TicketWorkSessions
                .AsNoTracking()
                .Where(session =>
                    session.AgentUserID == agent.ID &&
                    session.StartAt >= fromDate &&
                    session.StartAt < endExclusive &&
                    session.EndedAt != null);

            var agentTotalWorkMinutes = await agentWorkQuery
                .SumAsync(session => session.DurationMinutes ?? 0);

            var agentWorkSessionCount = await agentWorkQuery.CountAsync();
            var agentAverageWorkMinutes = agentWorkSessionCount == 0
                ? 0
                : Math.Round((double)agentTotalWorkMinutes / agentWorkSessionCount, 1);

            var commentsAdded = await _context.TicketComments
                .AsNoTracking()
                .CountAsync(comment =>
                    comment.UserID == agent.ID &&
                    comment.CreatedAt >= fromDate &&
                    comment.CreatedAt < endExclusive);

            var activityCount = await _context.TicketActivityLogs
                .AsNoTracking()
                .CountAsync(activity =>
                    activity.PerformedByUserID == agent.ID &&
                    activity.CreatedAt >= fromDate &&
                    activity.CreatedAt < endExclusive);

            var reassignments = await _context.TicketAssignments
                .AsNoTracking()
                .CountAsync(assignment =>
                    assignment.AgentUserID == agent.ID &&
                    assignment.UnassignedAt.HasValue &&
                    assignment.UnassignedAt.Value >= fromDate &&
                    assignment.UnassignedAt.Value < endExclusive);
        ...
        var recentTickets = await ticketsInPeriod

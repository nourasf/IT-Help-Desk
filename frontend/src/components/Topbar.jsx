...
function BellIcon() {
  return (
    <svg viewBox="0 0 24 24" fill="none" aria-hidden="true">
      <path d="M18 9a6 6 0 0 0-12 0c0 7-3 7-3 9h18c0-2-3-2-3-9Z" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" />
...
function Topbar() {
  ...
  function showNotificationToast(notification) {
    ...
    toastTimerRef.current = window.setTimeout(() => setToastNotification(null), 6500);
  }
  ...
  useEffect(() => {
    ...
    async function checkForNewNotifications() {
      try {
        ...
        setNotifications(items);
        setUnreadCount(Number(data.unreadCount || 0));
        if (newUnread) showNotificationToast(newUnread);
      } catch {}
    }
  ...
  return (
    <>
      <header className="topbar">
        <div className="topbar-search"><span className="search-icon">⌕</span><input type="text" placeholder="Search anything..." /></div>
        <div className="topbar-user">
          <div className="notification-center" ref={notificationRef}>
            <button className={`topbar-icon-button notification-bell ${unreadCount > 0 ? "has-unread" : ""}`} type="button" onClick={toggleNotifications} aria-label="Open notifications" aria-expanded={notificationsOpen}>
              <BellIcon />
              ...
              <section className="notification-popover" aria-label="Notifications">
                ...
                <div className="notification-list">
                  {notificationsLoading ? (
                    ...
                    <div className="notification-state error"><strong>Couldn’t load notifications</strong><p>{notificationError}</p><button type="button" onClick={loadNotifications}>Try again</button></div>
                  ) : notifications.length > 0 ? (
                    notifications.map((notification) => {
                      const visual = getNotificationVisual(notification.type);
                      return (
                        <button type="button" className={`notification-item ${notification.isRead ? "is-read" : "is-unread"}`} key={notification.id} onClick={() => openNotification(notification)}>
                          <span className={`notification-icon ${visual.tone}`}>{visual.icon}</span>
                          <span className="notification-copy">
                            <span className="notification-item-topline"><small>{visual.label}</small><time>{formatRelativeTime(notification.createdAt)}</time></span>
                            <strong>{notification.title}</strong><p>{notification.message}</p>
                            {notification.ticketId && <span className="notification-ticket-link">Open ticket <b>→</b></span>}
                          </span>
      ...
      {toastNotification && toastVisual && (
        <div className="notification-toast-wrap" role="status" aria-live="polite">
          <article className={`notification-toast ${toastVisual.tone}`}>
            ...
            <button type="button" className="notification-toast-main" onClick={() => openNotification(toastNotification)}>
              <span className="notification-toast-topline"><small>{toastVisual.label}</small><time>Just now</time></span>
              <strong>{toastNotification.title}</strong><p>{toastNotification.message}</p>
              {toastNotification.ticketId && <span className="notification-toast-link">View ticket →</span>}
            </button>
            <button type="button" className="notification-toast-close" aria-label="Dismiss notification" onClick={() => setToastNotification(null)}>×</button>
          </article>
        </div>
      )}
    </>

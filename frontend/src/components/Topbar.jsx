...
function getNotificationVisual(type) {
  ...
  if (normalized.includes("resolved")) {
    return { icon: "✓", tone: "success", label: "Resolved" };
  }

  if (normalized.includes("assigned") || normalized.includes("reassigned")) {
    return { icon: "↗", tone: "purple", label: "Assignment" };
  }

  if (normalized.includes("escalat")) {
    return { icon: "!", tone: "warning", label: "Escalated" };
  }

  if (normalized.includes("cancel")) {
    return { icon: "×", tone: "danger", label: "Cancelled" };
  }

  if (normalized.includes("reopen")) {
    return { icon: "↻", tone: "blue", label: "Reopened" };
  }

  if (normalized.includes("comment")) {
    return { icon: "✎", tone: "blue", label: "Comment" };
  }

  if (normalized.includes("created")) {
    return { icon: "+", tone: "purple", label: "New ticket" };
  }

  if (normalized.includes("closed")) {
    return { icon: "✓", tone: "neutral", label: "Closed" };
  }
    ...
    <header className="topbar">
      ...
      <div className="topbar-user">
        <div className="notification-center" ref={notificationRef}>
          <button
            className={`topbar-icon-button notification-bell ${
          ...
          {notificationsOpen && (
            <section className="notification-popover" aria-label="Notifications">
              ...
              <header className="notification-header">
                ...
                <div className="notification-header-orb" aria-hidden="true">
                  <span>✦</span>
                </div>
              </header>
              ...
              <div className="notification-toolbar">
                <span>
                  {recentUnread > 0 ? `${recentUnread} unread in this list` : "Latest activity"}
                </span>
                ...
                {notificationsLoading ? (
                  ...
                  notifications.map((notification) => {
                    ...
                    return (
                      ...
                      >
                        <span className={`notification-icon ${visual.tone}`}>
                          {visual.icon}
                        </span>
                        ...
                        <span className="notification-copy">
                          ...
                          {notification.ticketId && (
                            <span className="notification-ticket-link">
                              Open ticket <b>→</b>
                            </span>
                          )}
                        </span>

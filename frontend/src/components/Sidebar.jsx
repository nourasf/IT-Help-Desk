...
function SidebarIcon({ name }) {
  ...
  return <svg className="sidebar-svg-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">{icons[name]}</svg>;
}
...
function Sidebar({ activePage, collapsed, onToggle }) {
  ...
  const links = [
    { name: "dashboard", label: "Dashboard", path: dashboardPaths[role] || "/login", icon: "dashboard" },
    ...(role === "admin" ? [
    ...
    { name: "notifications", label: "Notifications", path: "/notifications", icon: "notification" },
    { name: "knowledge-base", label: "Knowledge Base", path: "/knowledge-base", icon: "knowledge" },
    { name: "ai-assistant", label: "AI Assistant", path: "/ai-assistant", icon: "ai" },
  ];

  return (
    <aside className={`sidebar ${collapsed ? "collapsed" : ""}`}>
      <div className="sidebar-top">
        <div className="sidebar-brand"><img src={logo} alt="SupportHub" className="sidebar-logo" /></div>
        <button type="button" className="sidebar-collapse" onClick={onToggle} aria-label={collapsed ? "Open sidebar" : "Close sidebar"}><SidebarIcon name="collapse" /></button>
      </div>
      <nav className="sidebar-nav">
        {links.map((link) => (
          <NavLink key={link.name} to={link.path} title={collapsed ? link.label : undefined} className={({ isActive }) => `nav-item ${activePage === link.name || isActive ? "active" : ""}`}>
            <span className="nav-icon"><SidebarIcon name={link.icon} /></span>

...
function TicketAttachmentImage({ ticketId, attachment }) {
  ...
  if (failed) return <div className="ticket-attachment-failed">{attachment.fileName}</div>;
  if (!src) return <div className="ticket-attachment-loading">Loading image...</div>;
  ...
  return (
    <div className="ticket-comment-attachments">
      {attachments.map((attachment) => (
        <a key={attachment.id} href={`http://localhost:5099${attachment.downloadUrl}`} target="_blank" rel="noreferrer" className="ticket-comment-attachment">
          {attachment.contentType?.startsWith("image/") ? <TicketAttachmentImage ticketId={ticketId} attachment={attachment} /> : <span>{attachment.fileName}</span>}
        </a>
      ))}
    </div>
  );
...
function TicketDetails() {
  ...
  async function loadTicket() { const data = await getTicketById(id); setTicket(data); setIsWorking(Boolean(data.activeWorkSession)); }
  async function loadComments() { setComments(await getTicketComments(id)); }
  async function loadAgents() { if (!isManager) return; const data = await getAssignmentOptions(); setAgents(data.agents || []); }
  async function loadTimeline() {
    if (isEmployee) { setActivity([]); setHistory([]); return; }
    setActivity(await getTicketActivity(id));
    if (isManager || isAdmin) setHistory(await getTicketHistory(id)); else setHistory([]);
  }
  async function loadNotes() { if (isEmployee) return; try { setNotes(await getInternalNotes(id)); } catch { setNotes([]); } }
  async function loadPage() {
    setLoading(true); setError("");
    try { await Promise.all([loadTicket(), loadComments(), loadAgents(), loadTimeline(), loadNotes()]); }
    catch (requestError) { setError(requestError.message || "The ticket could not be loaded."); }
  ...
  async function handleWork() {
    ...
    try { const result = isWorking ? await pauseWork(id) : await startWork(id); showMessage(result.message || "Work session updated."); await Promise.all([loadTicket(), loadTimeline()]); }
    catch (e) { showMessage(e.message, "error"); }
    finally { setWorkLoading(false); }
  }

  async function submitComment(event) {
    event.preventDefault(); if (!newComment.trim()) return;
    ...
    try {
      ...
      setNewComment(""); setCommentFiles([]); await Promise.all([loadComments(), loadTimeline()]);
    } catch (e) { showMessage(e.message, "error"); }
  ...
  async function submitReply(event, parentCommentId) {
    ...
    setReplySubmitting(true); showMessage("");
    try {
      ...
      setReplyText(""); setReplyFiles([]); setReplyingTo(null); await Promise.all([loadComments(), loadTimeline()]);
    } catch (e) { showMessage(e.message, "error"); }
    finally { setReplySubmitting(false); }
  }

  async function submitInternalNote(event) {
    event.preventDefault(); if (!newNote.trim()) return;
    ...
    try {
      ...
      setNewNote(""); setNoteFiles([]); await Promise.all([loadNotes(), loadTimeline()]);
    } catch (e) { showMessage(e.message, "error"); }
    finally { setNoteSubmitting(false); }
  }

  function openAction(name) {
    setAction(name); setActionNote(""); setActionsOpen(false); showMessage("");
  ...
  async function runAction() {
    ...
    if (["resolve", "escalate", "reopen", "stop"].includes(action) && !note) { showMessage("Please add a reason or note first.", "error"); return; }
    setActionLoading(true); showMessage("");
    try {
      let result; let returnedToManager = false;
      ...
      showMessage(result?.message || "Ticket updated."); setAction(""); setActionNote("");
  ...
  const combinedTimeline = [...activity.map((item) => ({ ...item, source: "Activity", at: item.createdAt })), ...history.map((item) => ({ ...item, source: "History", at: item.createdAt }))].sort((a, b) => new Date(b.at || 0) - new Date(a.at || 0));
        ...
        {(isAgent || isManager || isAdmin) && (
          <section className="ticket-action-bar ticket-action-bar-clear">
            <div className="ticket-action-bar-copy">
              ...
              <small>Workflow actions are grouped under More Actions. History opens separately so the page stays easy to scan.</small>
            </div>
            <div className="ticket-action-buttons">
              {!isEmployee && <button className="ticket-toolbar-button" type="button" onClick={() => { setActionsOpen(false); setHistoryOpen(true); }}>View History <span>{combinedTimeline.length}</span></button>}
              <div className="ticket-more-actions-wrap">
                ...
                {actionsOpen && (
                  <div className="ticket-more-actions-menu">
                    ...
                    {isAgent && !isResolved && !isClosed && !isCancelled && <button className="ticket-action resolve" type="button" onClick={() => openAction("resolve")}><strong>Resolve</strong><span>Mark the issue as solved and add resolution notes.</span></button>}
                    {isAgent && !isResolved && !isClosed && !isCancelled && <button className="ticket-action escalate" type="button" onClick={() => openAction("escalate")}><strong>Escalate</strong><span>Flag the ticket for higher attention.</span></button>}
                    {isAgent && !isResolved && !isClosed && !isCancelled && <button className="ticket-action cancel" type="button" onClick={() => openAction("stop")}><strong>Stop Working</strong><span>Cancel it or return it for reassignment.</span></button>}
                    {(isManager || isAdmin) && isResolved && <button className="ticket-action close" type="button" onClick={() => openAction("close")}><strong>Close Ticket</strong><span>Make the resolved ticket read-only.</span></button>}
                    {canReopen && <button className="ticket-action reopen" type="button" onClick={() => openAction("reopen")}><strong>Reopen Ticket</strong><span>Return the ticket to the active workflow.</span></button>}
                    {!((isAgent && !isResolved && !isClosed && !isCancelled) || ((isManager || isAdmin) && isResolved) || canReopen) && <p className="ticket-more-actions-empty">No workflow actions are available for this ticket status.</p>}
                  </div>
                )}
              </div>
            </div>
          ...
          <article className="ticket-details-placeholder-card ticket-comments-card">
            ...
            <div className="ticket-comments-list">
              {rootComments.length ? rootComments.map((comment) => {
                ...
                return <div className="ticket-comment-item" key={comment.id}><span className="ticket-comment-avatar">{getInitials(getCommentUser(comment))}</span><div className="ticket-comment-body"><div className="ticket-comment-meta"><strong>{getCommentUser(comment)}</strong><time>{formatDate(comment.createdAt)}</time></div><p>{comment.comment}</p><AttachmentList ticketId={id} attachments={comment.attachments} />
                  ...
                  {replyingTo === comment.id && canComment && <form className="ticket-reply-form" onSubmit={(event) => submitReply(event, comment.id)}><textarea value={replyText} onChange={(event) => setReplyText(event.target.value)} placeholder={`Reply to ${getCommentUser(comment)}...`} /><FileChips files={replyFiles} setFiles={setReplyFiles} /><div className="ticket-reply-actions"><label className="ticket-attach-button">Attach screenshot<input type="file" accept="image/png,image/jpeg,image/webp" multiple hidden onChange={(event) => { const files = Array.from(event.target.files || []); setReplyFiles((current) => [...current, ...files]); event.target.value = ""; }} /></label><button type="button" className="ticket-reply-cancel" onClick={() => { setReplyingTo(null); setReplyText(""); setReplyFiles([]); }}>Cancel</button><button type="submit" disabled={replySubmitting || !replyText.trim()}>{replySubmitting ? "Replying..." : "Reply"}</button></div></form>}
            ...
            {canComment && <form className="ticket-comment-form" onSubmit={submitComment}><textarea value={newComment} onChange={(event) => setNewComment(event.target.value)} placeholder="Write a comment..." /><FileChips files={commentFiles} setFiles={setCommentFiles} /><div className="ticket-comment-form-footer"><div><label className="ticket-attach-button">Attach screenshot<input type="file" accept="image/png,image/jpeg,image/webp" multiple hidden onChange={(event) => { const files = Array.from(event.target.files || []); setCommentFiles((current) => [...current, ...files]); event.target.value = ""; }} /></label><small>Visible to the employee and assigned IT agent.</small></div><button type="submit" disabled={commentSubmitting || !newComment.trim()}>{commentSubmitting ? "Posting..." : "Post Comment"}</button></div></form>}
          </article>
        </section>

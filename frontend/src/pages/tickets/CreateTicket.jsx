...
function CreateTicket() {
  ...
  async function handleAnalyzeTicket() {
    ...
    try {
      ...
      setAiError(error.message || "The AI assistant could not analyze this ticket.");
    } finally {
  ...
  async function handleSubmit(event) {
    ...
    if (!form.category || !form.priority) {
      setErrorMessage("Please select both a category and a priority.");
      return;
    }
    if (attachmentError) {
            ...
            <aside className="ticket-ai-panel">
              ...
              {!aiAnalysis && !isAnalyzing && (
                <div className="ai-empty-state">
                  <div className="ai-status-message"><span />Ready to analyze</div>
                  <p>Add a subject and description, then let AI suggest the best category and priority.</p>
                  <button type="button" className="apply-suggestion-button" onClick={handleAnalyzeTicket} disabled={!form.subject.trim() || !form.description.trim()}>✦ Analyze Ticket</button>
                </div>
              )}

              {isAnalyzing && (
                <div className="ai-analyzing-state">
                  <div className="ai-thinking-indicator"><span /><span /><span /></div>
              ...
              {aiError && (
                <div className="ai-analysis-error" role="alert">
                  {aiError}
                  <button type="button" onClick={handleAnalyzeTicket}>Try Again</button>
                </div>
              )}

              {aiAnalysis && !isAnalyzing && (
                <>
                  <div className="ai-status-message"><span />Analysis complete</div>

                  <div className="ai-suggestion-block">
                    <span className="ai-suggestion-label">Suggested category</span>
                    <div className="ai-suggestion-value">
                      <span className="suggestion-icon">▣</span>
                      <div><strong>{aiAnalysis.category}</strong><small>Based on your issue</small></div>
                    </div>
                  </div>

                  <div className="ai-suggestion-block">
                    <span className="ai-suggestion-label">Suggested priority</span>
                    <div className="ai-suggestion-value">
                      <span className="priority-dot" />
                      <div><strong>{aiAnalysis.priority}</strong><small>Estimated business impact</small></div>
                    </div>
                  </div>

                  <div className="ai-suggestion-block">
                    <span className="ai-suggestion-label">AI summary</span>
                    <div className="recommended-action-box">{aiAnalysis.summary}</div>
                  </div>

                  {aiAnalysis.suggestions?.length > 0 && (
                    <div className="ai-suggestion-block">
                      <span className="ai-suggestion-label">Try these first</span>
                      <div className="ai-troubleshooting-list">
                        {aiAnalysis.suggestions.map((suggestion, index) => (
                          <div className="ai-troubleshooting-item" key={`${suggestion}-${index}`}>
                  ...
                  <button type="button" className="apply-suggestion-button" onClick={handleApplySuggestion}>✦ Apply Category & Priority</button>
                  <button type="button" className="reanalyze-ticket-button" onClick={handleAnalyzeTicket}>Analyze Again</button>
                  <p className="ai-disclaimer">AI suggestions are optional. You can change the category and priority before sending.</p>
                </>
              )}
            </aside>

            <section className="ai-benefits-card">
              <h3>Why use AI?</h3>
              <div className="ai-benefit-item"><span>💡</span><div><strong>Smart Suggestions</strong><p>AI analyzes your issue and recommends the best fit.</p></div></div>
              <div className="ai-benefit-item"><span>⚡</span><div><strong>Faster Resolution</strong><p>Helps our agents resolve your tickets quicker.</p></div></div>
              <div className="ai-benefit-item"><span>☷</span><div><strong>Step-by-Step Help</strong><p>Get troubleshooting steps you can try right away.</p></div></div>
            </section>
          </div>

          <footer className="create-ticket-footer">
            <div className="supporthub-footer-brand"><span>◈</span><div><strong>SupportHub</strong><small>© 2026 All rights reserved.</small></div></div>
            <div className="create-ticket-actions">
              <button type="button" className="cancel-ticket-button" onClick={() => navigate("/employee-dashboard")}>Cancel</button>
              <button type="submit" className="send-ticket-button" disabled={isSubmitting || isLoadingOptions || Boolean(optionsError)}>
                {isSubmitting ? (selectedFiles.length > 0 ? "Creating & uploading..." : "Sending...") : "➤ Send Ticket"}
              </button>
            </div>
          </footer>
        </form>

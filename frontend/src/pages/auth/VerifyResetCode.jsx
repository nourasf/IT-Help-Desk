...
function VerifyResetCode() {
  ...
  function handlePaste(event) {
    ...
    if (!pasted) return;
    const updated = ["", "", "", "", "", ""];
    pasted.split("").forEach((digit, index) => { updated[index] = digit; });
    setDigits(updated);
        ...
        <form className="recovery-form" onSubmit={handleSubmit}>
          <div className="otp-inputs" onPaste={handlePaste}>
            {digits.map((digit, index) => (
              <input key={index} ref={(element) => { inputRefs.current[index] = element; }} type="text" inputMode="numeric" maxLength="1" className={digit ? "filled" : ""} value={digit} onChange={(event) => handleChange(index, event.target.value)} onKeyDown={(event) => handleKeyDown(index, event)} aria-label={`Digit ${index + 1}`} />
            ))}
          </div>
          <button type="submit" className="recovery-primary-button" disabled={isLoading}>{isLoading ? "Verifying..." : "Verify Code"}</button>
        </form>
        <button type="button" className="recovery-link-button" onClick={() => navigate("/forgot-password")}>Didn't receive a code? Send another</button>
      </section>
    </main>
  );
}
